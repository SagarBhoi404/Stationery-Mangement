...
public class login extends JFrame {
	...
	public login() {
		setIconImage(Toolkit.getDefaultToolkit().getImage(login.class.getResource("/img/stationery.png")));
		...
		JLabel serverl = new JLabel(ser);
		if (ser.equals("Server Status : ON")) {
			serverl.setForeground(Color.GREEN);
		} else {
			serverl.setForeground(Color.RED);
		}
		serverl.setHorizontalAlignment(SwingConstants.CENTER);
		serverl.setBounds(59, 460, 275, 14);
		contentPane.add(serverl);
		...
		lblNewLabel_6.setBounds(356, 11, 30, 14);
		...
		exitlabel.setIcon(new ImageIcon(login.class.getResource("/img/roundbtn.png")));
				...
				if (userf.getText().equals("") || passf.getText().equals("")) {
					...
					try {
						...
						if (r.next()) {
							...
							JOptionPane.showMessageDialog(null, "Invalid Login Details", "Warning",
									JOptionPane.WARNING_MESSAGE);
						}
					} catch (Exception e) {
						System.out.println(e);
						JOptionPane.showMessageDialog(null, e + "\n\n" + " Start Server and Restart Software",
		...
		lblNewLabel_1_1.setIcon(new ImageIcon(login.class.getResource("/img/lock.png")));
		...
		lblNewLabel_3.setIcon(new ImageIcon(login.class.getResource("/img/user100.png")));
